...
def find_assistant_tokens(tokenizer, target):
    """在分词后的 input_ids 中定位所有 assistant 回复的起始和结束位置('assistant' '<|im_end|>')"""
    result = []
    start_index = 0
    end_index = 0
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            '''从 JSON 中读取一条数据，包含图像 ID 和对话列表（human 和 assistant 交替）'''
            ...
            '''将原始图像标记 <image> 替换为多个 <|image_pad|>'''

            input_ids = tokenizer(text)['input_ids']
            indexs = find_assistant_tokens(tokenizer, input_ids)
            labels = len(input_ids) * [tokenizer.pad_token_id]
            for index in indexs:
                labels[index[0]:index[1]] = input_ids[index[0]:index[1]]  # 仅保留 assistant 部分的 token
